...
class AdminCrearRecursoViewModel @Inject constructor(
    ...
    fun onTituloChange(v: String) =
        update { copy(titulo = v, successMessage = null, errorMessage = null) }

    fun onTipoChange(v: String) =
        update { copy(tipo = v, successMessage = null, errorMessage = null) }

    fun onModuloChange(v: String) =
        update { copy(modulo = v, successMessage = null, errorMessage = null) }

    fun onAutorChange(v: String) =
        update { copy(autor = v, successMessage = null, errorMessage = null) }

    fun onDescripcionChange(v: String) =
        update { copy(descripcion = v, successMessage = null, errorMessage = null) }

    fun onUrlChange(v: String) =
        update { copy(url = v, successMessage = null, errorMessage = null) }

    fun onImageChange(v: String) =
        update { copy(image = v, successMessage = null, errorMessage = null) }

    fun setModuloInicial(modulo: String) {
        if (modulo.isBlank()) return
        _state.value = _state.value.copy(modulo = modulo)
    }
    ...
    fun guardarRecurso(autorId: String) {
        val s = _state.value
        ...
        if (
            ...
            _state.value = s.copy(
                errorMessage = "Completa título, tipo, módulo, autor y URL."
            )
            return
        }
        ...
        viewModelScope.launch {
            ...
            runCatching {
                ...
                _state.value = s.copy(
                    isSaving = false,
                    errorMessage = e.message ?: "Error guardando recurso."
                )
            }
        }
    }
